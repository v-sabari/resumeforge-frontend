...
const CustomBlock = ({ label, content }) => {
  ...
  const Title = () => <h2 className="text-xl font-bold text-gray-900 mb-3 flex items-center gap-3 break-after-avoid"><span className="w-8 h-0.5 bg-blue-500 shrink-0"></span><span className="break-words min-w-0">{label}</span></h2>;
  if (mode === 'bullets') {
    if (!items?.filter(Boolean).length) return null;
    return <div className="mb-6"><Title/><ul className="ml-11 space-y-1.5">{items.filter(Boolean).map((it,i)=><li key={i} className="text-gray-700 text-sm flex items-start break-inside-avoid"><span className="text-blue-400 mr-2 shrink-0">▸</span><span className="break-words min-w-0">{it}</span></li>)}</ul></div>;
  }
...
export const CreativeATSTemplate = ({ data }) => {
  ...
  const ST = ({ children }) => <h2 className="text-xl font-bold text-gray-900 mb-3 flex items-center gap-3 break-after-avoid"><span className="w-8 h-0.5 bg-blue-500 shrink-0"></span><span className="break-words min-w-0">{children}</span></h2>;
        ...
        <div key="experience" className="mb-6"><ST>Work Experience</ST>
          <div className="ml-11 space-y-4">{experience.map((e,i)=>(
            <div key={i} className="break-inside-avoid">
              ...
              {e.responsibilities?.length>0&&<ul className="space-y-1">{e.responsibilities.map((r,j)=><li key={j} className="text-gray-700 text-sm flex items-start break-inside-avoid"><span className="text-blue-400 mr-2 shrink-0">▸</span><span className="break-words min-w-0">{r}</span></li>)}</ul>}
            </div>
          ))}</div>
        </div>) : null;
        ...
        <div key="projects" className="mb-6"><ST>Key Projects</ST>
          <div className="ml-11 space-y-3">{projects.map((p,i)=>(
            <div key={i} className="break-inside-avoid">
              ...
              {p.highlights?.length>0&&<ul className="mt-1 space-y-0.5">{p.highlights.map((h,j)=><li key={j} className="text-gray-700 text-sm flex items-start break-inside-avoid"><span className="text-blue-400 mr-2 shrink-0">▸</span><span className="break-words min-w-0">{h}</span></li>)}</ul>}
            </div>
          ))}</div>
        </div>) : null;
