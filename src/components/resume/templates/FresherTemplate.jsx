...
const CustomBlock = ({ label, content }) => {
  ...
  if (mode === 'bullets') {
    ...
    return <div className="mb-5">{H}<ul className="space-y-1">{items.filter(Boolean).map((it,i)=><li key={i} className="flex items-start text-sm text-gray-700"><span className="text-blue-400 mr-2 shrink-0">▸</span><span className="break-words min-w-0">{it}</span></li>)}</ul></div>;
  }
...
export const FresherTemplate = ({ data }) => {
    ...
    switch (sec.key) {
      ...
      case 'education': return education?.length ? (
        <div key="education" className="mb-5"><SH>Education</SH>
          {education.map((e,i)=>(
            <div key={i} className="mb-3"><div className="flex justify-between items-start flex-wrap gap-1"><div className="min-w-0"><h3 className="font-bold text-gray-900 text-sm break-words">{e.degree}{e.field?` in ${e.field}`:''}</h3><div className="text-gray-600 text-sm break-words">{e.institution}</div>{e.gpa&&<div className="text-xs text-gray-500 break-words">Grade / CGPA: {e.gpa}</div>}{e.details&&<div className="text-xs text-gray-400 mt-0.5 break-words">{e.details}</div>}</div><span className="text-xs text-gray-500 whitespace-nowrap shrink-0">{e.year}</span></div></div>
          ))}
        </div>) : null;
        ...
        <div key="projects" className="mb-5"><SH>Projects</SH>
          {projects.map((p,i)=>(
            <div key={i} className="mb-3">
              ...
              {p.highlights?.length>0&&<ul className="mt-1 space-y-0.5">{p.highlights.map((h,j)=><li key={j} className="flex items-start text-sm text-gray-700"><span className="text-blue-400 mr-2 shrink-0">▸</span><span className="break-words min-w-0">{h}</span></li>)}</ul>}
            </div>
          ))}
        </div>) : null;
        ...
        <div key="experience" className="mb-5"><SH>Experience</SH>
          {experience.map((e,i)=>(
            <div key={i} className="mb-4">
              ...
              {e.responsibilities?.length>0&&<ul className="space-y-0.5">{e.responsibilities.map((r,j)=><li key={j} className="flex items-start text-sm text-gray-700"><span className="text-blue-400 mr-2 shrink-0">▸</span><span className="break-words min-w-0">{r}</span></li>)}</ul>}
            </div>
          ))}
        </div>) : null;
      ...
      case 'achievements': return achievements?.length ? (<div key="achievements" className="mb-5"><SH>Achievements &amp; Awards</SH><ul className="space-y-1">{achievements.map((a,i)=><li key={i} className="flex items-start text-sm text-gray-700"><span className="text-blue-400 mr-2 shrink-0">▸</span><span className="break-words min-w-0">{a}</span></li>)}</ul></div>) : null;
  ...
  return (
    <div className="resume-template fresher max-w-4xl mx-auto bg-white p-8 shadow-lg font-sans overflow-hidden">
      {personalInfo&&(
