...
const SH = ({ children }) => (
  <div className="text-[11px] font-bold uppercase tracking-widest pb-0.5 mb-1.5 border-b-2 border-gray-400 text-gray-500 break-after-avoid">
    {children}
  </div>
);
...
const CustomBlock = ({ label, content }) => {
  ...
  return (
    <div>
      <SH>{label}</SH>
      <p className="text-gray-700 leading-relaxed break-words font-serif">{text}</p>
    </div>
  );
};
...
export const TraditionalTemplate = ({ data }) => {
  ...
  const renderSection = (sec) => {
    ...
    switch (sec.key) {
      ...
      case 'summary':
        ...
        return <div key="summary"><SH>Professional Summary</SH><p className="text-gray-700 leading-relaxed break-words font-serif text-[11px]">{summary}</p></div>;
        ...
        return (
          <div key="skills">
            ...
            <div className="space-y-0.5">
              {(skills || []).map((s, i) => <p key={i} className="break-words text-gray-700 font-serif text-[11px]">{s}</p>)}
            </div>
          </div>
        );
            ...
            {(experience || []).map((e, i) => (
              <div key={i} className="mb-2 break-inside-avoid">
                <div className="flex justify-between gap-2">
                  <span className="font-semibold break-words min-w-0 font-serif text-[11px]">{e.position}{e.company ? ` — ${e.company}` : ''}</span>
                  <span className="text-gray-500 shrink-0 whitespace-nowrap">{e.duration}</span>
                </div>
                {(e.location || e.employmentType) && <div className="text-gray-500 text-[10px]">{[e.location, e.employmentType].filter(Boolean).join(' · ')}</div>}
                {e.summary && <p className="text-gray-600 mt-0.5 break-words font-serif text-[11px]">{e.summary}</p>}
            ...
            {(projects || []).map((p, i) => (
              <div key={i} className="mb-1.5 break-inside-avoid">
                <div className="flex flex-wrap items-baseline gap-x-2">
                  <span className="font-semibold break-words min-w-0 font-serif text-[11px]">{p.name}</span>
                  {p.role && <span className="text-gray-500 text-[10px] break-words">({p.role})</span>}
                </div>
                {p.technologies && <div className="text-gray-500 text-[10px] break-words">{p.technologies}</div>}
                {(p.link || p.github) && <div className="text-gray-400 text-[10px] break-all">{[p.link, p.github].filter(Boolean).join(' · ')}</div>}
                {p.description && <p className="text-gray-700 mt-0.5 break-words font-serif text-[11px]">{p.description}</p>}
              ...
              <div key={i} className="mb-1.5 break-inside-avoid">
                <div className="flex justify-between gap-2">
                  <div className="min-w-0">
                    <span className="font-semibold font-serif text-[11px]">{e.degree}</span>
                    {e.field && <span className="text-gray-600 font-serif text-[11px]"> in {e.field}</span>}
                    {e.institution && <div className="text-gray-500 font-serif text-[11px]">{e.institution}</div>}
                    {e.gpa && <div className="text-gray-400 text-[10px]">Grade: {e.gpa}</div>}
                    {e.details && <div className="text-gray-400 text-[10px] mt-0.5">{e.details}</div>}
                  </div>
        ...
        return (
          <div key="certifications"><SH>Certifications</SH>
            {(certifications || []).map((c, i) => (
              <div key={i} className="break-words break-inside-avoid"><span className="font-medium break-words font-serif text-[11px]">{c.name}</span>{c.issuer && <span className="text-gray-500"> — {c.issuer}</span>}{c.year && <span className="text-gray-400"> ({c.year})</span>}</div>
            ))}
          </div>
        );
      ...
      case 'languages':
        ...
        return <div key="languages"><SH>Languages</SH><p className="break-words font-serif text-[11px]">{(languages || []).join(' · ')}</p></div>;
  ...
  return (
    <div className="resume-template traditional font-serif text-[11px] leading-tight text-gray-900 bg-white px-6 space-y-3 overflow-hidden">
