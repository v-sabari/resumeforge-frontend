...
/* ── Milestones definition (mirrors backend reward engine) ── */
const MILESTONES = [
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${map[status] ?? map.PENDING}`}>
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card p-6">
        ...
        <div className="space-y-3">
          {MILESTONES.map(({ count, reward, icon, color }) => {
            ...
            return (
              <div key={count}
                ...
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full
                  ${earned ? 'bg-success-100 text-success-700'
            ...
            {[
              ...
              <p key={i} className="flex items-center gap-2 text-xs text-ink-400">
                <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full
                                 bg-brand-100 text-brand-700 text-[9px] font-bold">{i + 1}</span>
                {step}
              </p>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {rewards.map((r, i) => (
              <div key={i}
                ...
                <span className="badge-success text-[10px] shrink-0">Active</span>
              </div>
            ))}
          </div>
        </div>
